...
void createMainWindow() {
	...
	g_pFrame->Create(NULL, _T("HaloRadio安装向导"), UI_WNDSTYLE_FRAME, 0L, 0, 0, 0, 0);
...
void InitTBCIASkinEngine(HWND hwndParent, int string_size, char *variables, stack_t **stacktop, extra_parameters *extra)
{
	...
	extra->RegisterPluginCallback(g_hInstance, PluginCallback);
	{
		...
		g_pFrame->Create( NULL, _T("HaloRadio安装向导"), WS_OVERLAPPED | WS_SYSMENU | WS_MINIMIZEBOX, WS_EX_WINDOWEDGE, 0, 0, 0, 0);
		g_pFrame->CenterWindow();
